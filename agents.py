...
class ExperienceReplay(object):
	...
	def get_batch(self, batch_size=10):
		...
		if batch_size < len(self.memory):
			timerank = list(range(1, len(self.memory) + 1))
			p = timerank / np.sum(timerank, dtype=float)
			## p is the probability of selecting a particular experience
			batch_idx = np.random.choice(list(range(len(self.memory))), replace=False, size=batch_size, p=p)
...
class HeuristicSortAgent(BaseAgent):	
	...
	def get_all_actions(self, states):
		...
		sorted_idx = sorted(list(range(len(states))),
							key=lambda x: list(states[x][-self.histlen:]) + [states[x][-self.histlen - 1]])

		## Sorted Actions 
		sorted_actions = sorted(list(range(len(states))), key=lambda i: sorted_idx[i])
...
class HeuristicWeightAgent(BaseAgent):
	...
	def get_all_actions(self, states):
		...
		sorted_idx = sorted(list(range(len(states))), key=lambda x: sum(states[x] * self.weights))
		sorted_actions = sorted(list(range(len(states))), key=lambda i: sorted_idx[i])
		return sorted_actions
